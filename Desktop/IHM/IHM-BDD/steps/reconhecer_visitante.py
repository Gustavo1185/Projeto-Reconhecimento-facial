...
@when("a foto {foto_visitante} de um visitante for capturada")    
def when_foto_visitante_capturada(context, foto_visitante):
    visitante = simular_visita(foto_visitante)
    context.reconhecido, context.aluno = reconhecer_aluno(visitante, context.configuracao, context.gerador_dados_falsos)
